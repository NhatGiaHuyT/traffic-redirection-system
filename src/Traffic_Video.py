...
DEFAULT_MODEL_PATH = r"./src/ckpts/model_epoch_5.pt"
...
def draw_tracks(frame, tracks, scale_factor, 
                ...
                direction_detection=False, in_d5=False):
    """
    ...
    for track in tracks:
        ...
        # Draw bounding box
        if in_d5:
            # scale_x = desired_width / 640
            # scale_y = desired_height / 640
            # x1 = int(x1 * scale_x)
            # y1 = int(y1 * scale_y)
            # x2 = int(x2 * scale_x)
            # y2 = int(y2 * scale_y)
...
# 1. Model path
model_options = {
    "YOLOv8 (Default)": r"./src/ckpts/yolov8_5e.pt",
...
selected_model = st.sidebar.selectbox(
    lang_mapping[lang]["model_path_label"],
    list(model_options.keys()),
    index=0
)
# model_path = st.sidebar.text_input(lang_mapping[lang]["model_path_label"], value=DEFAULT_MODEL_PATH)
model, tracker = init_models(model_options[selected_model])

# 2. Video or Live Stream
video_options = {
    "Camera System 1 (Local Video)": r"./src/videos/Traffic.mp4",
    "Camera System 2": r"./src/videos/Traffic2.mp4",
    "Camera System 3": r"./src/videos/Traffic3.mp4",
    "Camera System 4": r"./src/videos/vecteezy_ho-chi-minh-city-traffic-at-intersection-vietnam_1806644.mov",
    # "Live Webcam (ID 0)": "webcam0",  # We'll handle "webcam0" as a special case
    ". NguyenBieu-CauChuY": "./src/frames_d5/5a82439f5058170011f6eaa9",
    ". NguyenVanCu-TranHungDao1": "./src/frames_d5/5b0b7aba0e517b00119fd800",
    ". NguyenTriPhuong-TranHungDao": "./src/frames_d5/5b632a79fd4edb0019c7dc0f",
    ". HongBang-NgoQuyen": "./src/frames_d5/5b632b60fd4edb0019c7dc12",
    ". HongBang-ChauVanLiem": "./src/frames_d5/5b728aafca0577001163ff7e",
    ". NguyenVanCu-TranHungDao3": "./src/frames_d5/5b632950fd4edb0019c7dc07",
    ". NguyenTriPhuong-TranPhu": "./src/frames_d5/5d8cd1f9766c880017188938",
    ". NguyenTriPhuong-NguyenTrai": "./src/frames_d5/5d8cd49f766c880017188944",
    ". HungVuong-NgoGiaTu": "./src/frames_d5/5deb576d1dc17d7c5515ad1f",
    ". HongBang-NgoQuyen2": "./src/frames_d5/5deb576d1dc17d7c5515ad20",
    ". VoVanKiet-NguyenVanCu1": "./src/frames_d5/56de42f611f398ec0c48128b",
    ". VoVanKiet-NguyenVanCu2": "./src/frames_d5/56de42f611f398ec0c48128c",
    ". VoVanKiet-CauChuY": "./src/frames_d5/56de42f611f398ec0c48128e",
    ". VoVanKiet-HuynhManDat": "./src/frames_d5/56de42f611f398ec0c48128f",
    ". VoVanKiet-AnBinh": "./src/frames_d5/56de42f611f398ec0c481290",
    ". VoVanKiet-NguyenTriPhuong1": "./src/frames_d5/56de42f611f398ec0c481291",
    ". TranHungDao-NguyenBieu": "./src/frames_d5/63ae78ccbfd3d90017e8f1f9",
    ". HongBang-PhuDongThienVuong": "./src/frames_d5/63b3c274bfd3d90017e9ab93",
    ". AnDuongVuong-TranPhu": "./src/frames_d5/66b1c1bf779f740018673ef2",
    ". HaiThuongLanOng-ChauVanLiem": "./src/frames_d5/662b4de41afb9c00172d85c5",
    ". HongBang-TaUyen": "./src/frames_d5/662b4e201afb9c00172d85f9",
    ". TranHungDao-TranPhu": "./src/frames_d5/662b4efc1afb9c00172d86bc",
    ". VoVanKiet-CauChuY1": "./src/frames_d5/5822adbbedeb6c0012a2d65f",
    ". HongBang-NguyenThiNho": "./src/frames_d5/5822f23aedeb6c0012a2d6a8",
...
set_background("./src/imgs/sakura.jpg")

# Video capture
def init_video_capture(video_source):
    if video_source == "webcam0":
        cap = cv2.VideoCapture(0)
    elif video_source.startswith("./src/frames_d5/"):
        return None
    else:
...
if video_options[selected_video].startswith("./src/frames_d5/"):
    # If reading from a directory of frames, set the first frame
    current_frame = 0
    cam_dir = os.path.join(video_options[selected_video])
    frame_files = sorted(os.listdir(cam_dir))
    frame_files = [os.path.join(cam_dir, f) for f in frame_files if f.endswith('.jpg')]
while (cap and cap.isOpened()) or frame_files:
    ...
    if cap:
        ...
        # stop if we reach the end of the frames, or change the selected video
        if current_frame >= len(frame_files) or st.session_state.get("selected_video") != selected_video:
            break
        print(f"Processing frame {frame_files[current_frame]} - {current_frame + 1}/{len(frame_files)}")
        frame = cv2.imread(frame_files[current_frame])
    ...
    frame = draw_tracks(frame, tracks, 
                        ...
                        direction_detection=direction_detection, in_d5=cap is None)
    ...
    # end of processing 1 frame
    time_cost = time.time() - start_loop_time
    if time_cost < 5 and cap is None:
        time.sleep(5 - time_cost) # Sleep to maintain 0.2 fps (5 seconds per frame)
